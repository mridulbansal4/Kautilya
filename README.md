...
<img src="kautilyalogo.png" alt="KAUTILYA" width="150"/>
...
[![Guardrails](https://img.shields.io/badge/Guardrails-Structural-5c35cc?style=for-the-badge)](#the-governed-reasoning-spine--its-guardrails)
...
<img src="docs/assets/hero-stage.png" alt="Kautilya persona stage — one governed engine, the Rajesh money-shot nudge" width="100%"/>
...
- [Architecture](#architecture-the-backend-thinks-the-frontend-renders) · [The governed reasoning spine](#the-governed-reasoning-spine--its-guardrails) · [Repository layout](#repository-layout)
...
| Grounding | the model's parametric memory | **governed retrieval** — no KFS document ⇒ fail closed, no nudge |
...
three live personas — flip between them in real time with the persona switcher. The **reasoning path
and governance are identical** across all three; only the surfaced action and its presentation adapt.

<table>
<tr>
<td width="33%" valign="top"><img src="docs/assets/persona-rajesh.png" alt="Rajesh, 42 — RecommendSIP ₹250"/></td>
<td width="33%" valign="top"><img src="docs/assets/persona-aarav.png" alt="Aarav, 21 — MicroSIP ₹100, gamified"/></td>
<td width="33%" valign="top"><img src="docs/assets/persona-mohan.png" alt="Mohan, 68 — SeniorFD + scam-shield, 1.3× type"/></td>
</tr>
<tr>
<td valign="top"><b>Rajesh, 42</b> — mid-career, ₹40k idle surplus. <code>RecommendSIP</code> (₹250) + <code>OfferTermCover</code>. Formal / advisory, <b>2</b> choices.</td>
<td valign="top"><b>Aarav, 21</b> — college student. <code>RecommendMicroSIP</code> (₹100) + <code>SuggestCreditBuilder</code>. Hinglish, gamified <b>DCS ring + streak</b>, <b>3</b> choices.</td>
<td valign="top"><b>Mohan "Dadaji", 68</b> — senior pensioner. <code>SuggestSeniorFD</code> + <code>ScamShieldAlert</code> + <code>EscalateToRM</code>. Font <b>1.3×</b>, spacious, high-contrast, <b>1</b> choice.</td>
</tr>
</table>
...
Every hop is logged and rendered live in the **reasoning-path inspector** — toggle *"Show reasoning
path"* and watch the same governed spine light up for every persona:

<div align="center">
<img src="docs/assets/reasoning-path.png" alt="Reasoning-path inspector — Intent → Policy → Retrieval → Ontology → Rules → NBA → Explainer → HITL" width="100%"/>
</div>
...
└── docs/                                  # architecture · ontology · personas · yono_features · demo_script
    └── assets/                            # the screenshots embedded in this README
```
...
The `/ops` desktop console surfaces four executive views — **Activation**, **Barrier Twin**,
**AI Observability**, and **Admin**. The Activation view tracks the *Dormant → Active* pipeline that
ticks the moment a nudge is confirmed through the HITL gate:

<div align="center">
<img src="docs/assets/ops-dashboard.png" alt="/ops Activation dashboard — North-Star scorecard + Dormant→Active pipeline" width="100%"/>
</div>
...
> The screenshots in this README are captured from the running frontend (`VITE_MOCK=1`). The FastAPI
> governed spine ships its ontology schema, seed, and contract; wiring the live `/v1` endpoints to the
> frontend is the current build focus — the mock seam is byte-identical to the real one.
